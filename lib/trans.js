...
exports.create = function(req, res, next) {
  ...
  transForm.promise(req)
    ...
    .then(function(ob) {
      request.post(ob.callback_url, {
          signed_notice: notices.signedQs(ob),
        }, function (error, response, body) {
          // In case of a real implementation, you should queue and retry.
          console.log(error, response.statusCode, response, body);
          throw next(new Error('Postback against ' + ob.callback_url + ' failed: ' + error));
        });
    })
    .fail(function(err) {
      next(err);
