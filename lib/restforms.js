      ...
      newForm.validate(function(err, form) {
        if (err) {
          ...
          if (ignoredKeys.length) {
            reject(new errors.InvalidArgumentError(
                {message: 'Unrecognized fields: ' + ignoredKeys.toString()}));
          } else {
            resolve(form.data);
          }

        } else {
          reject(new errors.InvalidArgumentError({message: JSON.stringify(getErrors(form))}));
        }
      });
    });
  };
