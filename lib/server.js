    ...
    server.pre(function authenticate() {
      return function (req, res, next) {
        passport.authenticate('token', { session: false }, function(err, user, info) {
          if (config.unprotectedUrls.indexOf(req.url) !== -1) {
            options.log.info('Skipping auth for whitelisted URL ' + req.url);
            return next();
          }
          if (!user.authenticated) {
            var errorMessage = 'Authentication failed: ' + info;
            options.log.info(errorMessage);
            return next(new restify.InvalidCredentialsError(errorMessage));
          }
          next();
        })(req, res, next);
      };
