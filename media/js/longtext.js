 ...
 */
define('longtext', ['jquery'], function() {
  var $doc = $(document);
  ...
  function checkOverflow(el, allowForEllipsis) {
    ...
    allowForEllipsis = allowForEllipsis === true ? 10 : 0;
    var $el = $(el);
    hidden.text($el.text());
  ...
  $.fn.checkLongText = function($longTextElms, allowForEllipsis) {
    ...
    if ($elmsToCheck.is(':hidden')) {
      console.log('[longtext] one or more elements was hidden. Bailing');
      return;
    }
    ...
    if ($elmsToCheck.length && currentVpWidth !== vpWidth) {
      ...
      $elmsToCheck.each(function() {
        if (checkOverflow(this, allowForEllipsis) === true) {
          console.log('[longtext] Longtext found.');
          hasLongText = true;
          return false;
        }
      });
      ...
      console.log('[longtext] currentVpWidth matches vpWidth so noop.');
    }
